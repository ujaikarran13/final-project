...
//		jdbcTemplate.update(sql);
	...
	public void exerciseProblem21() {
		checkAnswerForProblem("21", "TIP: This one can be a little tricky. If you're off by one, look closer at each clause of your statement. There's something you can change to get a different result set.");
	}

	private void checkAnswerForProblem(String problemNumber) {
		checkAnswerForProblem(problemNumber, "");
	}

	private void checkAnswerForProblem(String problemNumber, String hint) {
		...
		testSuite(rsExpected, rsActual, hint);
	}

	private void testSuite(ResultSet rsExpected, ResultSet rsActual, String hint) {
		...
		checkRowCount(rsExpected, rsActual, hint);
	...
	private void checkRowCount(ResultSet rsExpected, ResultSet rsActual, String hint) {
		try {
			...
			if (hint.isBlank()) {
				...
				assertEquals("Expected row count doesn't match actual row count." + System.lineSeparator() + hint + System.lineSeparator(),
						rowCountExpected, rowCountActual);
			}
